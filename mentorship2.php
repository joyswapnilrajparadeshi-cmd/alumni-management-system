...
// Fetch mentors from the database using PDO
try {
    $query = "SELECT id, name, description, image_path FROM mentors";
...
<body>
    ...
    <main>
        ...
        <!-- Upload Mentor Form (Visible only to Admins) -->
        <?php if ($isAdmin): ?>
    ...
    <script>
        ...
        function moveSlide(step) {
            ...
            if (currentIndex >= mentorSlides.length) {
                currentIndex = 0;
            }
            if (currentIndex < 0) {
                currentIndex = mentorSlides.length - 1;
            }
