# 🎓 Alumni Management System | Enterprise Full Stack Platform  

[![PHP](https://img.shields.io/badge/PHP-777BB4?style=for-the-badge&logo=php&logoColor=white)]
[![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)]
[![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)]
[![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)]
[![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)]
[![PHPMailer](https://img.shields.io/badge/PHPMailer-ff69b4?style=for-the-badge)]
[![Status](https://img.shields.io/badge/Status-Production%20Ready-brightgreen?style=for-the-badge)]

---

## 🚀 Project Overview  

**Alumni Management System** is a **large-scale enterprise-grade full stack platform** built to digitize, automate, and streamline alumni engagement for universities and colleges.

It provides a **secure, scalable, and role-based ecosystem** enabling:

- Seamless alumni interaction  
- Centralized institutional management  
- Secure donations & payment processing  
- Automated communication & notifications  
- Career & mentorship networking  

> Designed with **production scalability, security, and performance** in mind.

---

## 🎯 Core Functionalities  

### 👨‍🎓 Alumni Panel  
Secure Registration & Login System

Profile Management

Job Applications

Event Viewing & Registration

Mentorship Program Access

Donations System

Video Gallery Access

Contact Admin

Notification Alerts

---

### 🛠 Admin Panel  
Secure Admin Authentication

Alumni Record Management

Event Management (Add / Update / Delete)

Mentor Management (Add with Image Upload)

Donation Monitoring

Job Post & Application Management

Video Management (Upload & Delete)

Manual Email Notification System

Full Database Control

---

## 🏗 System Architecture  


Frontend (HTML, CSS, JavaScript)
↓
Backend (PHP + Flask Microservices)
↓
Database (MySQL)
↓
SMTP Services (PHPMailer)
↓
Payment Gateway (Razorpay)


---

## 🔐 Security & Access Control  

- Role-based authentication (Admin / Alumni)  
- Secure session handling  
- OTP-based password reset  
- CSRF-safe form handling  
- Secure payment flow integration  

---

🧩 Modules Implemented
Module	Description
👤 User Authentication	Secure Alumni & Admin Login
🎉 Events	Event creation & participation
💼 Jobs	Job posting & applications
🤝 Mentorship	Mentor listing with images
💰 Donations	Donation data handling
🎬 Videos	Video gallery & deletion
🔔 Notifications	Admin-triggered email alerts
📩 Contact	Query & feedback system

## 🛠 Technology Stack  

### Backend  
- PHP 8+  
- Flask (Python microservices)  
- PHPMailer  
- Razorpay API  

### Database  
- MySQL  

### Frontend  
- HTML5  
- CSS3  
- JavaScript  

### Tools & Infrastructure  
- XAMPP  
- Composer  
- Node.js  
- GitHub  

---

## 🖼 Platform Screenshots  

### 🏠 Landing Page  
<img width="1899" height="850" alt="image" src="https://github.com/user-attachments/assets/ba4db9cd-dcd6-4d79-8c34-387befca0afb" />
<img width="1901" height="520" alt="image" src="https://github.com/user-attachments/assets/784868c0-70f5-4fde-a73e-b95096093eed" />



---

## 📁 Project Structure  


alumni-management-system/
├── admin/ # Admin dashboard & controls
├── alumni/ # Alumni dashboard
├── assets/ # CSS, JS, images
├── uploads/ # Media uploads
├── vendor/ # Composer dependencies
├── razorpay-php-master/ # Payment SDK
├── PHPMailer-master/ # SMTP library
├── app.py # Flask microservice
├── db_connection.php # Database config
├── alumni.sql # Database schema
└── README.md


---

## ⚙ Installation & Setup

### 1️⃣ Clone Repository
git clone https://github.com/joyswapnilrajparadeshi-cmd/alumni-management-system.git

### 2️⃣ Import Database
- **Create Database:** `alumni`  
- **Import File:** `alumni.sql`

### 3️⃣ Configure Database
Edit `db_connection.php`:

$server = "localhost";  
$user   = "root";  
$pass   = "";  
$db     = "alumni";

### 4️⃣ Start Server
- Start **Apache & MySQL** using **XAMPP**

### 5️⃣ Access System
Open in your browser:  
[http://localhost/alumni-management-system/](http://localhost/alumni-management-system/)

---


## 👨‍💻 Author

Paradeshi Joy Swapnil Raj
B.Tech CSE | Full Stack Developer | AI & ML Enthusiast

**📧 Email**: joyswapnilrajparadeshi@gmail.com

**🌐 Portfolio**: https://joyswapnilrajparadeshi-cmd.github.io/portfolio/

**⭐ Support**

If you find this project useful, please star ⭐ the repository and support my work!
